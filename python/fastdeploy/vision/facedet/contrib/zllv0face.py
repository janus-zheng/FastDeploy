...
class Zllv0FacePreprocessor:
    def __init__(self):
        """Create a preprocessor for Zllv0Face
        """
        self._preprocessor = C.vision.facedet.Zllv0Preprocessor()

    def run(self, input_ims):
        """Preprocess input images for Zllv0Face
...
class Zllv0FacePostprocessor:
    def __init__(self):
        """Create a postprocessor for Zllv0Face
        """
        self._postprocessor = C.vision.facedet.Zllv0FacePostprocessor()

    def run(self, runtime_results, ims_info):
        """Postprocess the runtime results for Zllv0Face
...
class Zllv0Face(FastDeployModel):
    def __init__(self,
                 model_file,
                 params_file="",
                 runtime_option=None,
                 model_format=ModelFormat.ONNX):
        """Load a Zllv0Face model exported by Zllv0Face.

        :param model_file: (str)Path of model file, e.g ./zllv0face.onnx
        :param params_file: (str)Path of parameters file, e.g yolox/model.pdiparams, if the model_fomat is ModelFormat.ONNX, this param will be ignored, can be set as empty string
        :param runtime_option: (fastdeploy.RuntimeOption)RuntimeOption for inference this model, if it's None, will use the default backend on CPU
        :param model_format: (fastdeploy.ModelForamt)Model format of the loaded model
        """
        super(Zllv0Face, self).__init__(runtime_option)

        self._model = C.vision.facedet.Zllv0Face(
            model_file, params_file, self._runtime_option, model_format)

        assert self.initialized, "Zllv0Face initialize failed."
    ...
    def preprocessor(self):
        """Get Zllv0Preprocessor object of the loaded model

        :return Zllv0Preprocessor
        """
        return self._model.preprocessor

    @property
    def postprocessor(self):
        """Get Zllv0Postprocessor object of the loaded model

        :return Zllv0Postprocessor
        """
